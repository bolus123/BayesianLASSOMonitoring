...
//' get a sample from inv gaussian distribution
 ...
 arma::mat fillValue(arma::mat A, arma::uvec a, double b){
 ...
 // [[Rcpp::export]]
 Rcpp::List updateTauGammaGrp(arma::colvec Y, 
                              arma::mat Tau,
                              arma::mat Gamma, double muq, double sigma2, double pho, double xi2,
                              arma::mat Group,
                              arma::mat uniGrp, int nGrp, 
                              int T, int q, arma::mat D, arma::mat H_, int Hflg, int m){
   
  
   arma::mat tmpTau = Tau;
   arma::mat tmpGamma = Gamma;
   ...
   if (Hflg == 1) {
     ...
     for (kk = 0; kk < nGrp; kk++) {
     //for (kk = 0; kk < 1; kk++) {
       //Rcpp::Rcout << "k:" << kk << std::endl;
       // update TauGrp
       tmpIdx = arma::find(Group == uniGrp(kk));
       //Rcpp::Rcout << tmpIdx << std::endl;
       
       rr = tmpIdx.n_elem;

       Hnot = removeCols(H_, tmpIdx);
       Ht = H_.cols(tmpIdx);
       
       Taunot = removeRows(tmpTau, tmpIdx);
       Taut = tmpTau.rows(tmpIdx);
       //Rcpp::Rcout << Taut << std::endl;
       
       Gammanot = removeRows(tmpGamma, tmpIdx);
       Gammat = tmpGamma.rows(tmpIdx);
       //Rcpp::Rcout << Gammanot << std::endl;
       //Rcpp::Rcout << 3.4 << std::endl;
       //Rcpp::Rcout << Gammat << std::endl;
       //Rcpp::Rcout << 3.5 << std::endl;
       
       zetanot = zeta0 - Hnot * (Taunot % Gammanot);
       //Rcpp::Rcout << Hnot * (Taunot % Gammanot) << std::endl;
       zetat = zetanot - Ht * Gammat;
       //Rcpp::Rcout << zetat << std::endl;
       //Rcpp::Rcout << Ht * Gammat << std::endl;
       //Rcpp::Rcout << 3.6 << std::endl;
       
       tmp = arma::exp(-1.0 / 2.0 / sigma2 * zetanot.t() * D * zetanot);
       tmpzetanot = tmp(0);
       //Rcpp::Rcout << tmpzetanot << std::endl;
       
       tmp = arma::exp(-1.0 / 2.0 / sigma2 * zetat.t() * D * zetat);
       tmpzetat = tmp(0);
       //Rcpp::Rcout << tmpzetat << std::endl;
       
       //Rcpp::Rcout << 3.7 << std::endl;
       
       if (tmpzetat == 0 & tmpzetanot == 0) {
         p = pho;
       } else {
         tmp = pho * tmpzetat / (pho * tmpzetat + (1 - pho) * tmpzetanot);
         p = tmp(0);
       }
       
       //Rcpp::Rcout << p << std::endl;
       //Rcpp::Rcout << 3.8 << std::endl;
       pvec(kk) = p;
       
       tmpval = R::rbinom(1, p);
       //Rcpp::Rcout << "tmpval:" << tmpval << std::endl;
       //Rcpp::Rcout << TauGrp << std::endl;
       TauGrp(kk) = tmpval;
       //Rcpp::Rcout << TauGrp << std::endl;
       //Rcpp::Rcout << 3.9 << std::endl;
       
       for (jj = 0; jj < rr; jj++) {
         tmpTau(tmpIdx(jj), 0) = tmpval;
       }
       
       //Rcpp::Rcout  << tmpTau.rows(tmpIdx) << std::endl;
       
       if (tmpval == 1.0) {
         DHt = Ht.t() * D;
         //Rcpp::Rcout << 4.1 << std::endl;
         
         tHtDHt = DHt * Ht;
         //Rcpp::Rcout << 4.2 << std::endl;
         
         Gammathat = getInv(tHtDHt) * DHt * zetanot;
         //Rcpp::Rcout << 4.3 << std::endl;
         
         tmpvar =  DHt * Ht / sigma2 + (1.0 / xi2 * tmpeye.eye(rr, rr)); 
         //Rcpp::Rcout << 4.4 << std::endl;
         
         tmpmean = getInv(tmpvar) * (1.0 / sigma2) * tHtDHt * Gammathat;
         //Rcpp::Rcout << 4.5 << std::endl;
         
         tmp = rmvnorm(tmpmean, tmpvar);
         //Rcpp::Rcout << 4.6 << std::endl;
         
         for (jj = 0; jj < rr; jj++) {
           tmpGamma(tmpIdx(jj), 0) = tmp(jj, 0);
         }
         //Rcpp::Rcout << tmpGamma << std::endl;
         
         //Rcpp::Rcout << 4.7 << std::endl;
       } else {
         for (jj = 0; jj < rr; jj++) {
           tmpGamma(tmpIdx(jj)) = R::rnorm(0, sqrt(xi2));
         }
       }
   ...
   Rcpp::List out = Rcpp::List::create(
     Rcpp::_["Tau"] = tmpTau,
     Rcpp::_["Gamma"] = tmpGamma,
     Rcpp::_["p"] = pvec,
     Rcpp::_["TauGrp"] = TauGrp
   );
   return(out);
 }
 ...
 //' @param theta2 is a given shape2 of the prior beta distribution for the probability of Tau.
 //' @param xi2 is a given variance of the prior normal distribution for shifts.
 //' @param theta01 is a given shape1 of the prior beta distribution for the probability of Tau0.
 //' @param theta02 is a given shape2 of the prior beta distribution for the probability of Tau0.
 //' @param thetaGrp1 is a given shape1 of the prior beta distribution for the probability of TauGrp.
 //' @param thetaGrp2 is a given shape2 of the prior beta distribution for the probability of TauGrp.
 //' @param method is a choice of methods including MT(McCulloch-Tsay), regression, LASSO, ALASSO(Adaptive LASSO), MonoLASSO(LASSO with Monotonicity constrains), MonoALASSO(Adaptive LASSO with Monotonicity constrains).
 //' @param bound0 is a lower bound of the methods with Monotonicity constrains.
 //' @param boundqplus1 is  a upper bound of the methods with Monotonicity constrains.
 //' @param nsim is the number of samples draw from MCMC.
 ...
 //' @param tol is the tolerance.
 //' @param H is the design matrix for shifts.
 //' @param Group is the design vector for grouping shifts.
 //' @export
 //' @examples
 //' T <- 100
 //' q <- 5
 ...
 //' 
 // [[Rcpp::export]]
 Rcpp::List GibbsRFLSMGrp(arma::colvec& Y,int& q, 
                          arma::mat& A, double& a, double& b, double& alpha, double& beta, 
                          double& theta1, double& theta2, double& xi2,
                          Rcpp::String& method, double& bound0, double& boundqplus1,
                          int& nsim, int& by, int& burnin,
                          ...
                          Rcpp::Nullable<Rcpp::NumericMatrix> Group = R_NilValue) {
   ...
   // Calculate Group
   if (Group.isNotNull()) {
     Group_ = Rcpp::as<arma::mat>(Group);
   } else {
     Group_.zeros();
   }
   
   arma::mat uniGrp = arma::unique(Group_);
   int nGrp = uniGrp.n_rows;
   ...
   for (ii = 0; ii < TotalSim; ii++) {
     ...
     TauGamma = updateTauGammaGrp(Y, 
                                  Tau, 
                                  Gamma, muq, sigma2, pho, xi2,
                                  Group_,
                                  uniGrp, nGrp, 
                                  T, q, D, H_, Hflg, m);
     ...
     Gamma = Rcpp::as<arma::mat>(TauGamma["Gamma"]);
     pvec = Rcpp::as<arma::mat>(TauGamma["p"]);
     TauGrp = Rcpp::as<arma::mat>(TauGamma["TauGrp"]);
     ...
     //#update pho
     if (Hflg == 1) {
       tmpval = arma::accu(TauGrp);
       pho = R::rbeta(theta1 + nGrp, theta2 + nGrp - tmpval);
     }
     
     if (ii >= burnin) {
       if (ii % by == 0) {
         Phiout.col(aa) = Phi;
         sigma2out.col(aa) = sigma2;
         Tauout.col(aa) = Tau;
         Gammaout.col(aa) = Gamma;
         muqout.col(aa) = muq;
         Muout.col(aa) = Mu;
         pout.col(aa) = pvec;
         TauGrpout.col(aa) = TauGrp;
         phoout.col(aa) = pho;
         eta2out.col(aa) = eta2;
         lambda2out.col(aa) = lambda2;
         aa = aa + 1;
       }
     }
